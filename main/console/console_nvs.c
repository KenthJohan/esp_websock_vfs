...
static esp_err_t get_value_from_nvs(nvs_handle_t nvs, const char *key, nvs_type_t type, char out_buf[], int out_buf_size)
{
	...
	case NVS_TYPE_I8: {
		int8_t value;
		e = nvs_get_i8(nvs, key, &value);
		if (e != ESP_OK) {
			break;
		}
		snprintf(out_buf, out_buf_size, "%d", value);
	} break;

	case NVS_TYPE_U8: {
		uint8_t value;
		e = nvs_get_u8(nvs, key, &value);
		if (e != ESP_OK) {
			break;
		}
		snprintf(out_buf, out_buf_size, "%u", value);
	} break;

	case NVS_TYPE_I16: {
		int16_t value;
		e = nvs_get_i16(nvs, key, &value);
		if (e != ESP_OK) {
			break;
		}
		snprintf(out_buf, out_buf_size, "%d", value);
	...
	case NVS_TYPE_I32: {
		int32_t value;
		e = nvs_get_i32(nvs, key, &value);
		if (e != ESP_OK) {
			break;
		}
		snprintf(out_buf, out_buf_size, "%" PRIi32 "", value);
	} break;

	case NVS_TYPE_U32: {
		uint32_t value;
		e = nvs_get_u32(nvs, key, &value);
		if (e != ESP_OK) {
			break;
		}
		snprintf(out_buf, out_buf_size, "%" PRIu32 "", value);
	} break;

	case NVS_TYPE_I64: {
		int64_t value;
		e = nvs_get_i64(nvs, key, &value);
		if (e != ESP_OK) {
			break;
		}
		snprintf(out_buf, out_buf_size, "%lld", value);
	} break;

	case NVS_TYPE_U64: {
		uint64_t value;
		e = nvs_get_u64(nvs, key, &value);
		if (e != ESP_OK) {
			break;
		}
		snprintf(out_buf, out_buf_size, "%llu", value);
	} break;

	case NVS_TYPE_STR: {
		size_t len;
		e = nvs_get_str(nvs, key, NULL, &len);
		if (e != ESP_OK) {
			break;
		}
		char *str = (char *)malloc(len);
		e = nvs_get_str(nvs, key, str, &len);
		if (e == ESP_OK) {
			snprintf(out_buf, out_buf_size, "%s", str);
		}
		free(str);
	} break;
	...
	case NVS_TYPE_BLOB: {
		size_t len;
		e = nvs_get_blob(nvs, key, NULL, &len);
		if (e != ESP_OK) {
			break;
		}
		snprintf(out_buf, out_buf_size, "blob:%iB", len);
	} break;
...
#define FORMAT_NVS_LIST "%-20.20s %-20.20s %-20.20s %20.20s\n"
static int list(const char *part, const char *name, const char *str_type)
{
	...
	e = nvs_entry_find(part, NULL, type, &it);
	if (e == ESP_ERR_NVS_NOT_FOUND) {
		ESP_LOGE(__func__, "No such entry was found");
		return 1;
	} else if (e != ESP_OK) {
		ESP_LOGE(__func__, "NVS error: %s", esp_err_to_name(e));
		return 1;
	}

	printf(FORMAT_NVS_LIST, "namespace", "key", "type", "value");
	...
	while (1) {
		...
		nvs_handle_t nvs = 0;
		e = nvs_open(info.namespace_name, NVS_READONLY, &nvs);
		if (e != ESP_OK) {
			ESP_LOGE(__func__, "nvs_open() %s", esp_err_to_name(e));
			continue;
		}

		char buf[64] = {0};
		e = get_value_from_nvs(nvs, info.key, info.type, buf, sizeof(buf));
		if (e != ESP_OK) {
			ESP_LOGE(__func__, "get_value_from_nvs() %s", esp_err_to_name(e));
			memset(buf, 0, sizeof(buf));
		}

		nvs_close(nvs);
		printf(FORMAT_NVS_LIST, info.namespace_name, info.key, type_to_str(info.type), buf);
...
static int get_value(int argc, char **argv)
{
	...
	nvs_type_t type = str_to_type(str_type);
	if (type == NVS_TYPE_ANY) {
		ESP_LOGE(__func__, "Type '%s' is undefined", str_type);
		return ESP_ERR_NVS_TYPE_MISMATCH;
	}
	nvs_handle_t handle;
	e = nvs_open(current_namespace, NVS_READONLY, &handle);
	if (e != ESP_OK) {
		ESP_LOGE(__func__, "%s", esp_err_to_name(e));
		return 1;
	}
	e = get_value_from_nvs(handle, key, type, buf, sizeof(buf));
	if (e != ESP_OK) {
		nvs_close(handle);
		ESP_LOGE(__func__, "%s", esp_err_to_name(e));
		return 1;
